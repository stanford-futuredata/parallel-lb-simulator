...
with open('randomBoxPlot.txt') as f:
	for line in f:
		...
		if "p99:" in components:
			...
			skew = float(components[components.index("Skew:") + 1])

			if skew == 0.2 and trialNo > 20:
				random[0].append(p99)
			elif skew == 0.4:
				random[1].append(p99)
			elif skew == 0.6:
...
bp = ax.boxplot(result, patch_artist = True, vert = 1, notch = False, showfliers=False, whis = (10, 90))
 
# x-axis labels
ax.set_xticklabels(["0.2", "0.2", "0.4", "0.4", "0.6", "0.6", "0.8", "0.8"])
...
fig.savefig('skewVsLatencySolr.pdf', format='pdf')
